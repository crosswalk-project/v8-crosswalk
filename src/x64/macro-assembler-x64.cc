...
#define SIMD128_HEAP_ALLOCATE_FUNCTIONS(V) \
  V(Float32x4, float32x4, FLOAT32x4, float32, 4)       \
  V(Float64x2, float64x2, FLOAT64x2, float64, 2)       \
  V(Int32x4, int32x4, INT32x4, int32, 4)

#define DECLARE_SIMD_HEAP_ALLOCATE_FUNCTION(Type, type, TYPE, typebase,    \
                                            length)                        \
void MacroAssembler::Allocate##Type(Register result,                       \
  ...
  movp(FieldOperand(scratch1, FixedTypedArrayBase::kMapOffset),            \
       kScratchRegister);                                                  \
  movp(scratch3, Immediate(length));                                       \
